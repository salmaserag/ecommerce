...
class CategoriesDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function ($model) {
            ...
            $html .= '<a class="btn btn-sm btn-primary me-2" href="' . route('categories.edit', ['category' => $model->id]) . '">
                          <i class="bi bi-pencil"></i>
                      </a>';
            $html .= '<form action="' . route('categories.destroy', ['category' => $model->id]) . '" method="POST" style="display:inline-block;">
                          ' . csrf_field() . '
                          ' . method_field('DELETE') . '
            ...
            ->editColumn('name', function ($model) {
                ...
                $html = '<a href=' . route('categories.show', ['category' => $model->id]) . '>' .  $model->name . '</a>';
                return $html;
            })
